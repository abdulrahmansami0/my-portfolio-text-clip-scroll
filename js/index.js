...
const applyCustomEffect_3 = (contentElement) => {
  ...
  const imageUrls = ["img/power-sun.jpg", "img/moon.jpg", "img/tree-light.jpg"];
  let currentIndex = 0;
...
const applyCustomEffect_10 = (contentElement) => {
  // Select relevant elements for the effect
  const clipPath = contentElement.querySelectorAll("svg clipPath");
  const poster = contentElement.querySelectorAll(".poster__container");
  const posterCard = contentElement.querySelectorAll(".poster__card");

  // Animation logic for each clipPath element
  [...posterCard].forEach((clipPathEl, pos) => {
    gsap
      .timeline({
        scrollTrigger: {
          trigger: posterCard[pos], // Triggers based on poster element
          start: "top bottom", // Start animation when poster is at top of viewport
          end: "bottom top", // End animation when poster is at bottom of viewport
          scrub: true, // Allows smooth scrubbing effect
        },
      ...
      .fromTo(
        clipPathEl,
        {
          // xPercent: pos % 2 ? 40 : -80, // Initial xPercent value based on position
          // skewX: pos % 2 ? (-50, -40) : (50, 40),
          scale: 0.1,
          // filter: "saturate(0%)",
          // transformOrigin: "top right",

          filter: "blur(1px)", // Initial blur value (no blur)
          opacity: 0,
        },
        {
          ease: "none",
          // xPercent: pos % 2 ? -40 : 20, // Target xPercent value based on position
          // skewX: pos % 2 ? (50, 40) : (-50, -40),
          scale: 1,
          // filter: "saturate(100%)",
          // transformOrigin: "top right",
  ...
  contentWithSVG.forEach((contentElement, position) => {
    switch (position) {
      ...
      case 8:
        ...
        break;
      case 9:
        applyCustomEffect_10(contentElement);
      default:
